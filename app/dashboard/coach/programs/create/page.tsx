...
export default function CreateProgramPage() {
  ...
  const [meals, setMeals] = useState<Meal[]>([
    { id: '1', meal_name: 'Breakfast', description: '', calories: '' }
  ]);
  ...
  function updateTrainingDay(dayId: string, field: string, value: string) {
    setTrainingDays(trainingDays.map(d =>
  ...
  function updateExercise(dayId: string, exerciseId: string, field: string, value: string | number) {
    setTrainingDays(trainingDays.map(d =>
      d.id === dayId
  ...
  function addMeal() {
    setMeals([...meals, { id: Date.now().toString(), meal_name: '', description: '', calories: '' }]);
  }
  ...
  function updateMeal(mealId: string, field: string, value: string) {
  ...
  async function handleCreateTrainingProgram(e: React.FormEvent) {
    ...
    const { data: program, error: programError } = await supabase
      ...
      .insert({
        ...
        description: trainingProgramDescription || null,
      })
      .select()
      .single();
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'training' | 'diet')}>
              ...
              <TabsContent value="training">
                <form onSubmit={handleCreateTrainingProgram} className="space-y-6">
                  {error && (
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setTrainingProgramDescription(e.target.value)}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={addTrainingDay}>
                      ...
                      <Card key={day.id}>
                        <CardHeader>
                          <div className="flex items-center justify-between">
                            <Input
                              value={day.day_name}
                              onChange={(e) => updateTrainingDay(day.id, 'day_name', e.target.value)}
                              className="max-w-xs"
                            />
                ...
                <form onSubmit={handleCreateDietPlan} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="e.g., 2000 Calorie Meal Plan"
                      required
                      disabled={loading}
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      id="dietDescription"
                      value={dietPlanDescription}
                      onChange={(e) => setDietPlanDescription(e.target.value)}
                      ...
                      <Card key={meal.id}>
                        <CardContent className="pt-6 space-y-3">
                          ...
                          <Textarea
                            ...
                            placeholder="Meal description and ingredients"
                          />
                          <Input
                            type="number"
                            value={meal.calories}
